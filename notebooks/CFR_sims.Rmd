...
transformed parameters {
 ...
 expected_riskset = rep_matrix(0.0, T,L);  // initialize expected_riskset to all 0s;
  for (j in 1:L) {
    ...
    expected_riskset[1,j] = c[1,j]+0.0001; //risk set on day 1 at inf duration 1 is just c1
    expected_deaths[1,j] = expected_riskset[1,j] * h1 + 0.0001 ;
    expected_recovereds[1,j] = expected_riskset[1,j] * h2 + 0.0001;
 

    for (t in 2:T) {
      expected_riskset[t,j] = (expected_riskset[t-1,j] + c[t,j] - (expected_riskset[t-1,j]*h1 -  (expected_riskset[t-1,j]* h2))+0.0001);
      expected_deaths[t,j] = expected_riskset[t,j] *  h1 + 0.0001 ;
      expected_recovereds[t,j] = expected_riskset[t,j] *  h2 + 0.0001;
    } 
  }
...
parameters {
  real loglambda1[L]; //parameter for time to death distribution
  real loglambda2; // parameter for time to death or recovery distribution
  //real delta1_hubei; //parameter for diff in hubei's lambda1
  //real delta2_hubei; // parameter for diff in hubei's lambda2
 ...
 for(i in 1:length(lambda1_hubei)){
  try (
    rho_wbl_hubei[i] <- integrate(function(x) {   
                                          risk.Inf <-lambda1_hubei[i]*exp(- ((lambda1_hubei[i]*x +lambda2_hubei[i]*x)))
                                          return(risk.Inf)
    },
                        lower = 0, upper =Inf)$value)
 }
...
model {
  ...
  //individual data
    for (g in 1:Nd) {
      target+=log(alpha1*exp(loglambda1 + delta1_hubei*wdi[g]) * td[g]^(alpha1-1) *exp(-(exp(loglambda1 + delta1_hubei*wdi[g])*td[g]^alpha1+exp(loglambda2 + delta2_hubei*wdi[g])*td[g]^alpha2)));//log(h1(t)*S(t-))
      }
    for (h in 1:Nr){
     //target+= log(indiv_expect_recover[tr[h]]);
      target+=log(alpha2*exp(loglambda2 + delta2_hubei*wri[h])*tr[h]^(alpha2-1)*exp(-(exp(loglambda1 + delta1_hubei*wri[h])*tr[h]^alpha1+exp(loglambda2 + delta2_hubei*wri[h])*tr[h]^alpha2)));
    }
}
