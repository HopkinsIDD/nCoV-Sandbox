...
```{r, warning=, message=FALSE}
   ...
   jhucsse <- read_JHUCSSE_cases("2020-03-12 23:59", 
                                 append_wiki = TRUE)
  ...
  #Load in the JHU CSSE Data
todaydate <- ISOdate(2020,3,12, hour = 11, min = 59, tz = "UTC")
...
deaths <- IndDat %>% filter(delta == 1 & !is.na(t) & t>1) %>% select(t, province)
recovs <-   IndDat %>% filter(delta == 2& !is.na(t)& t>1) %>% select(t, province)
td <- deaths$t
wdi <- ifelse(deaths$province != "Hubei" | is.na(deaths$province), 0, 1)
tr <- recovs$t
wri <- ifelse(recovs$province != "Hubei" | is.na(recovs$province), 0, 1)

cfrmdl_CH_data$tr <- tr
cfrmdl_CH_data$td <- td
cfrmdl_CH_data$wdi <- wdi
cfrmdl_CH_data$wri <- wri
cfrmdl_CH_data$Nd <- length(td)
cfrmdl_CH_data$Nr <- length(tr)
