...
|---:|:-------------------------:|:------------------------------------------------------------:|
| 1  | Traditional               |                                                              |
| 2  | Rapid Testing and Tracing | Scenario 1, but individuals are isolated on average 2 days faster than traditional contact tracing. |
| 3  | Passive Scale-up          | Scenario 1, but we assume a scale-up in testing increases passive detection 2-fold |
| 4  | Immediate Isolation       | Suspected symptomatic cases and their household are asked to isolate upon first contact with a healthcare provider |
...
3. **What happens if we test more people based on symptoms?**

This scenario is the same as Scenario 1 described above, but we now assume that 20% of infectious individuals are identified, isolated, and reached for contact tracing.
...
This scenario is the same as Scenario 1 described above, but we now assume that infectious individuals are isolated, on average, on the day of symptom onset, when they might first contact a healthcare provider. Household contacts are isolated on the same day which, following the serial interval of SARS-CoV-2, will be on average 1 day after they are exposed (4 days *before* symptom onset). Non-household contacts are still actively traced following confirmed test results.
...
5. **How quickly do contacts need to be identified, before there are few additional benefits to isolation?**

Below, we plot $R_{eff, p}$, the effective reproductive number among contacts of cases detected through passive surveillance, and $R_{eff}$, the effective reproductive number considering all infected individuals, while varying the median time from infection to isolation for household and non-household contacts of a confirmed case. We assume (as in Scenario 4) that cases are, on average, passively detected on day of symptom onset and isolated, along with their household members, on the same day.
...
6. **What proportion of community contacts need to be identified to achieve a certain reduction in transmission?**
...
This plot shows $R_{eff}$, the effective reproductive number considering all infected individuals, while varying the proportion of all cases that are identified through passive surveillance (including asymptomatic or non-clinically apparent cases). We note that Scenarios 1 and 2 differ only in the proportion of cases detected, and therefore appear identical here.
...
ggplot(outP, aes(x=propP, y=Re_I1, group=as.factor(scn), color=as.factor(scn))) +
        ...
        geom_hline(aes(yintercept=1), lty=2, col='gray') +
        theme_minimal() +
        xlab("proportion cases detected in passive surveillance") +
        ylab("R_eff") +
        scale_color_discrete(name="Scenario")
...
There is evidence that attack rates among household contacts are nearly 10 times higher than attack rates among non household contacts ([Bi et al.](https://www.thelancet.com/journals/laninf/article/PIIS1473-3099(20)30287-5/fulltext#tbl3)). Below, we plot $R_{eff,p}$ across different values of $\nu$, the relative reproductive number in household contacts compared to non-household contacts, across varying levels of $\rho_H$, the proportion of household contacts successfully isolated, and $\rho_C$, the proportion of non-household contacts successfully isolated. We show results for Scenario 2 (Rapid Isolation) and Scenario 4 (Immediate Isolation). The red line shows $R_{eff,p}=1$

```{r nu comparison, fig.width = 8, fig.height = 10}
...
8. **How does the effective reproductive number change as a function of the delays to isolating household and non-household contacts?**

Below, we plot $R_{eff,p}$ across different average delays from symptom onset to isolation of household and non-household contacts of confirmed cases underotherwise Traditional contact tracing (Scenario 1). This represents an optimistic scenario where 90% of household contacts and 75% of non-household contacts are traced and isolated. 

```{r scn1 gamma comparison, fig.width = 5, fig.height = 10}
...
delaysC <- seq(0, 7, 0.05)
delaysH <- seq(0, 7, 0.05)
relInfs <- c(1, 4, 10)
...
ggplot(out, aes(x=delayC, y=delayH, fill=Re_I1_passive)) +
  ...
  theme_bw()+
  scale_fill_viridis_c(name="R_eff,p") +
  xlab("Time from symptom onset to non-HH isolation") +
  ylab("Time from symptom onset to HH isolation")

```

This shows the trade-offs between proportion of household cases isolated and the average delay from symptom onset to isolation of infected household contacts. This assumes an optimistic scenario where 75% of non-household contacts are effectively isolated on day of symptom onset.
...
for(i in 1:length(delaysH)){
  ...
  for(j in 1:length(propH)){
    ...
    for(n in 1:length(relInfs)){
        p_base$nu <- relInfs[n]
        out <- rbind(out,
                    c(p_base$delaysH[i], p_base$rhoH, p_base$nu, calcRe_exact(p_base)))
    }
  }
}
...
ggplot(out, aes(x=delayH, y=rhoH, fill=Re_I1_passive)) +
  geom_tile() +
  geom_contour(aes(z=Re_I1_passive, 
                   color = factor(..level.. == 1, levels = c(F, T), labels = c("", "Reff=1")))) + 
  scale_colour_manual(values = c("white", "red"), guide=F) +
  ...
  theme_bw()+
  scale_fill_viridis_c(name="R_eff,p") +
  xlab("Time from symptom onset to HH isolation") +
  ylab("Proportion HH contacts isolated")

```

Here, we plot $R_{eff,p}$ across different average delays from symptom onset to isolation of non-household contacts under the Immediate Isolation strategy (Scenario 4) and proportion of non-household contacts reached. This represents an optimistic scenario where 90% of household contacts are quarantined with the index case on day of symptom onset.

```{r scn4 gamma comparison, fig.width = 5, fig.height = 10}
...
ggplot(out, aes(x=delayC, y=rhoC, fill=Re_I1_passive)) +
  geom_tile() +
  geom_contour(aes(z=Re_I1_passive, 
                   color = factor(..level.. == 1, levels = c(F, T), labels = c("", "Reff=1")))) + 
  scale_colour_manual(values = c("white", "red"), guide=F) +
  ...
  scale_fill_viridis_c(name="R_eff,p") +
  xlab("Time from symptom onset to non-HH isolation") +
  ylab("Proportion non-HH contacts isolated")
