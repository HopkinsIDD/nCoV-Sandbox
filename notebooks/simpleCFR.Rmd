...
todaydate <- ISOdate(2020,5,7, hour = 11, min = 59, tz = "UTC")
...
IndDat <- process_MK_linelist(IndDat, date)
...
IndDat2 <- read.verity.data()  
IndDat2 <- IndDat2 %>% 
  mutate(country2=recode(country, "USA"="US")) %>%
  mutate(loc = locs[as.character(country2)]) %>% 
  select(t, country2, loc, delta)

deaths1 <- IndDat %>% filter(delta == 1 & !is.na(t) & t>=1) %>% select(t, country2, loc)
recovs1 <-   IndDat %>% filter(delta == 2& !is.na(t)& t>=1) %>% select(t, country2, loc)
deaths2 <- IndDat2 %>% filter(delta == 1 & !is.na(t) & t>=1) %>% select(t, country2, loc)
recovs2 <-   IndDat2 %>% filter(delta == 2& !is.na(t)& t>=1) %>% select(t, country2, loc)

deaths <- rbind(deaths1, deaths2)
recovs <- rbind(recovs1, recovs2)
...
# function to estimate logical bounds on CFR over time
# output num censored, lower and upper bounds
cfrbounds.tv <- function(data){
  dat <- data
 # results <- matrix(ncol = 3, nrow = nrow(dat$c))
  c <- cumsum(rowSums(dat$c))
  d <- cumsum(rowSums(dat$d))
  r <- cumsum(rowSums(dat$r))
  censored <- c - (d+r)
  lower <- d/c # assume all censored are not deaths
  upper <- (d + censored)/c # assume all censored are deaths later that day
  bounds <- cbind(lower, upper)
  results <- cbind(censored, bounds)
  return(results)
}


days <- c(1:nrow(cfr_moddata$c))
bounds <- cfrbounds.tv(cfr_moddata)
bounds

#losing like a million cases. remove smoothing? 

plot(days, bounds[,2], type = "l", ylim = c(0,1))
lines(days, bounds[,3], col = "blue")


dat <- cfr_moddata

cumsum(rowSums(cfr_moddata$c))
...
ggplot(test, aes(x=x, y=ys, colour = loc, group=loc))+
  geom_line()
ch <- incidence_data %>% filter(Incidence>30000)
...
# test <- cfrbounds(cfr_moddata[nrow(cfr_moddata$c),])
# test
#starting values
