    ...
    for (t in 2:T) {
      ...
      for(k in 2:V){
       ...
       h2[k] = alpha2*lambda2^alpha2*k^(alpha2-1);
        ...
        expected_deaths[t,j] +=  (k<=t)*(expected_riskset[k,t] * h1[k]) +0.0001;
        expected_recovereds[t,j] += (k<=t)*(expected_riskset[k,t] * h2[k] * (dr)^w[j]) +0.0001;
...
todaydate <- ISOdate(2020,2,11, hour = 00, min = 23, tz = "UTC")
...
mod2_all <- moddata(todaydate, ChinaOnly = TRUE, withHubei = TRUE, HubeiOnly = FALSE)
...
```{r, eval = FALSE}
  cfrmdl_je_res <- sampling(cfrmdl2, data=mod2_hubei,
                         iter=3000)
