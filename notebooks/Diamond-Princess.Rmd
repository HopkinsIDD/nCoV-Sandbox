...
R0_shadow_dist_plot <- R_sims %>% 
    ...
    ungroup() %>% 
    mutate(gray_out=if_else(type=="SARS_est", dates>=ymd("2020-02-05"),
                            dates>=ymd("2020-02-08"))) %>% 
    ggplot(aes(x=dates, color=type, fill=type)) +
    geom_vline(aes(xintercept=ymd("2020-02-05")), linetype="dashed") +
    geom_vline(aes(xintercept=ymd("2020-02-11")), linetype="dashed") +
    geom_text(aes(x=ymd("2020-02-13"), y=32, label="Protocol\nchange"),
              color="black") +
    geom_vline(aes(xintercept=ymd("2020-02-16")), linetype="dashed") +
    geom_hline(aes(yintercept=1), linetype="dashed") +
...
Under these assumptions, we see that the peak reproduction number (and thus transmission) likely occurred prior to quarantine.



The National Institute of Infectious Diseases [stated](https://www.japantimes.co.jp/news/2020/02/20/national/coronavirus-diamond-princess-before-quarantine/) that they would release their own report soon and noted that 33 individuals were symptomatic prior to Feb 5th, which was not in the original surveillance data.
