...
With the COVID-19 estimate, the reproductive number falls below 1 three days after the implementation of the quarantine (Feb 8), which should be the goal of a quarantine.
But the reproductive number bounced up to 6 on Feb 10 before settling in between 2-3 until falling below 2 on Feb 18.
...
sym_plot_dat <- tibble(dates=c(R_SARS_sym$dates[2:26],
                           R_ncov_sym$dates[2:26],
                           R_eyeball_sym$dates[2:26]),
...
bind_rows(plot_dat, sym_plot_dat) %>%
    ...
    ggtitle("Comparison of overall and symptomatic analyses using COVID-19 estimates") +
    theme_bw() +
    theme(axis.text.y=element_text(color="black"),
          axis.text.x=element_text(color="black", angle=45, hjust=1),
          legend.position="bottom")
