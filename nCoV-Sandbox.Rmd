...
## 2020-20-01 Case Fatality Ratio
...
transformed parameters {
  ...
  real <lower=0> expected_deaths[T,L];
  ...
  for (j in 1:L) {
    for (t in 1:T) {
      expected_deaths[t,j] = 0;
        for (k in 1:t) {
          expected_deaths[t,j] += c[k,j] * rho * 
              (exponential_cdf(t-k+1,alphaD)-
                  exponential_cdf(t-k,alphaD));
        }
        //print(t);
        //print(j);
        //print(expected_deaths[t,j]);
        //print(rho);
        //print(d[t,j]);
        //print("--")
    }
