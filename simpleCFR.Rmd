...
data {
  ...
  real <lower= 0, upper=1> dr[T,L]; // prrobability of detectinga recovery.
}
...
transformed parameters {
 ...
 for (j in 1:L) {
    for (t in 1:T) {
      expected_deaths[t,j] = n[t,j] * lambda1 + 0.0001 ;
      expected_r[t,j] = n[t,j] * (lambda - lambda1) * dr[t,j] + 0.0001;
