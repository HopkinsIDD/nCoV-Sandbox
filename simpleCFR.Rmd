...
for (j in 1:L) {
    ...
    for (t in 2:T) {
      expected_riskset[j,t,1] = c[t,j]-c[t-1,j]+0.0001;
      ...
      for(k in 2:t){
        ...
        pk = (h1[k]+h2[k])/((alpha1*lambda1*t^(alpha1+1)/(alpha1+1)) + (alpha2*lambda2*t^(alpha2+1)/(alpha2+1))); // h(k) / H(t)
        expected_riskset[j,t,k] = expected_riskset[j, t-1, k-1] - pk*(expected_deaths[t-1,j] +  expected_riskset[j, t-1, k-1]* (h2[k-1]));
        expected_deaths[k,j] += expected_riskset[j,t,k] * h1[k]  ;
        expected_recovereds[k,j] += expected_riskset[j,t,k] * (h2[k]) * (dr)^w[j];
