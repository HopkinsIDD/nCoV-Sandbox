...
Time fixed hazards, but keeping track of risk sets (intermediate step)
...
transformed parameters {
  real <lower=0> expected_deaths[T,L]; //  # deaths at time T
  real <lower=0> expected_deaths[V,L]; //  # deaths at time T
  real <lower=0> expected_recovereds[T,L]; //  # those who have exited due to recovery at T
  //real <lower=0> expected_riskset[T,L]; // expected people in risk set
  real <lower=0> expected_riskset[L,T,V]; // expected people in risk set in inf dur timescale
  //real <lower=0> h1[V];
...
for (j in 1:L) {
    ...
    expected_riskset[j,1,1] = c[1,j]+0.0001; //risk set on day 1 at inf duration 1 is just c1
    expected_deaths[1,j] = expected_riskset[j,1,1] * lambda1 + 0.0001 ;
    expected_recovereds[1,j] = expected_riskset[j,1,1] * lambda2 * (dr)^w[j] + 0.0001;
 

    for (t in 2:T) {
      expected_riskset[j,t,1] = c[t,j]+0.0001;
      expected_deaths[t,j] = expected_riskset[j,t,1] * lambda1 + 0.0001 ;
      expected_recovereds[t,j] = expected_riskset[j,t,1] * lambda2 * (dr)^w[j] + 0.0001;
     // h1[1] = lambda1;
     // h2[1] = lambda2;
      ...
      for(k in 2:t){
       // h1[k] =lambda1;
       // h2[k] = lambda2;
       // pk[k] = (h1[k]+h2[k])/((lambda1+lambda2)*(t)); // h(k) / H(t) (integrated from 0 to t). or, what proportion of all of the deaths we saw today are likely among people in this risk cohort?
        expected_riskset[j,t,k] = expected_riskset[j, t-1, k-1] - (expected_riskset[j, t-1, k-1]*lambda1 +  (expected_riskset[j, t-1, k-1]* lambda2))+0.0001;
        expected_deaths[k,j] += expected_riskset[j,t,k] * lambda1 ;
        expected_recovereds[k,j] += expected_riskset[j,t,k] * lambda2 * (dr)^w[j];
        //print(c[t,j])
...
library(tidyverse)
  ...
  date <- ISOdate(2020,2,5)
   jhucsse <- read_JHUCSSE_cases("2020-02-05 16:43", 
                                 append_wiki = TRUE)
