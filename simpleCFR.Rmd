...
transformed parameters {
  real <lower=0> expected_deaths[T,L]; //  # deaths BY time T
  real <lower=0> expected_r[T,L]; //  # those who have exited due to recovery BY T


 for (j in 1:L) {
    for (t in 1:T) {
      expected_deaths[t,j] = n[t,j] * lambda1 + 0.00000001 ;
      expected_r[t,j] = n[t,j] * (lambda - lambda1) + 0.00000001;
...
library(tidyverse)
  ...
  ######### I DON'T UNDERSTAND WHAT WE ARE DOING HERE SINCE
  ######### RAW DATA WAS CUMLATIVE AND WE TIED TO BACK OUT
  ######### INCIDENE.....
  ...
  cfrmdl_je_data <- list(T=Tmax, L=L,
                      c = confirmeds,
                      d = totdeaths,
                      r = recovered,
                      e = cumexit,
...
####NOT SURE HOW TO RESOLVE THIS #######
  ##we now have everything to run our stand model 2
  cfrmdl_je_data <- list(T=Tmax, L=L,
                      c = confirmeds,
                      d = deaths,
                      r = recovered,
...
```{r, eval = FALSE}
  cfrmdl_je_res <- sampling(cfrmdl_je, data=cfrmdl_je_data,
                         iter=5000, init=1)
